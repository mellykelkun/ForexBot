...
class AdaptiveAIEngine:
    ...
    SYSTEM_PROMPT = (
        ...
        "- AFTER a losing streak (2+ losses), be MORE selective before any BUY/SELL:\n"
        "  * Prefer mtf_synthesis.alignment = ALIGNED\n"
        "  * At least 3 timeframes with confluence.score >= 70\n"
        "  * confidence >= 0.65\n"
        "  * RSI not in extreme overbought/oversold zone\n"
        ...
        "- If current_streak contains '1+ consecutive losses' → add +0.05 to min confidence.\n"
        "- If current_streak contains '2+ consecutive losses' → add +0.10 to min confidence "
        "and require confluence.score >= 75 on 3+ timeframes.\n"
        "- If current_streak contains '3+ consecutive losses' → prefer HOLD unless "
        "confluence.score >= 80 on 4+ timeframes with strong alignment.\n"
        "- If win_rate_pct < 40% → require confluence.score >= 75 on the majority of "
        "timeframes showing a signal.\n"
        ...
        "- Spread filtering is already handled by the system — do NOT penalize for spread.\n\n"

        # ── Confidence scoring ──
        "== CONFIDENCE SCORING ==\n"
        "confidence is a float 0.0–1.0. Calculate it systematically:\n"
        "- Start at 0.50 (neutral baseline).\n"
        "- +0.10 if mtf_synthesis.alignment = ALIGNED.\n"
        "- +0.05 per timeframe with confluence.score >= 70 (max +0.25 for 5 TFs).\n"
        ...
        "- -0.05 if volatility.squeeze = true on ANY timeframe (use caution but still tradeable).\n"
        "- -0.05 if my_trade_history.stats.win_rate_pct < 45%.\n"
        "- -0.05 per consecutive loss in current_streak (max -0.15).\n"
        "MAXIMUM possible confidence = ~0.95. Required minimum = 0.65.\n"
